...
<style>
    ...
    /* NOVO: Cards de Variação Temporal */
    .variacao-card {
...
def carregar_configuracao_banco():
    ...
    # Configurações disponíveis em ordem de prioridade
    configs = [
        {
            'nome': 'Supabase Cloud',
        ...
        },
        {
            ...
            'port': int(os.getenv('RENDER_PORT', '5432')),
            'sslmode': 'require',
            'connect_timeout': 10
        }
    ]
    ...
    if os.path.exists('.env'):
        try:
            ...
            # Se python-dotenv não estiver instalado, ler manualmente
            try:
    ...
    for config in configs:
        # Pular configurações incompletas
        ...
        try:
            ...
            # Criar configuração limpa (sem campos extras)
            config_teste = {k: v for k, v in config.items() 
            ...
            print(f"✅ Conectado com sucesso: {config['nome']}")
            return config_teste
            
        except psycopg2.OperationalError as e:
            error_msg = str(e)
            if "Cannot assign requested address" in error_msg:
                print(f"⚠️ {config['nome']}: Servidor inacessível (rede/firewall)")
            elif "authentication failed" in error_msg:
                print(f"⚠️ {config['nome']}: Credenciais inválidas") 
            elif "does not exist" in error_msg:
                print(f"⚠️ {config['nome']}: Base de dados não existe")
            else:
                print(f"⚠️ {config['nome']}: {error_msg[:100]}...")
            continue
            
        except Exception as e:
            print(f"⚠️ {config['nome']}: Erro inesperado - {str(e)[:100]}...")
            continue
    
    # Se todas falharam, usar configuração padrão local
    print("⚠️ Usando configuração padrão (localhost)")
    return {
        'host': 'localhost',
        'database': 'dashboard_baker', 
        'user': 'postgres',
...
def aba_insercao_banco():
    ...
    with st.form("form_busca_cte"):
        ...
        if buscar_btn and numero_busca:
            sucesso, resultado = buscar_cte_postgresql(numero_busca)
    ...
    if 'cte_encontrado' in st.session_state and 'numero_cte_edicao' in st.session_state:
        ...
        with st.form("form_edicao_cte"):
            ...
            with col1:
                ...
                veiculo_edit = st.text_input("Placa do Veículo", value=dados_cte.get('veiculo_placa', ''))
            ...
            with col2:
                ...
                numero_fatura_edit = st.text_input("Número da Fatura", value=dados_cte.get('numero_fatura', ''))
                data_baixa_edit = st.date_input("Data de Baixa", 
                                                value=processar_data_para_input(dados_cte.get('data_baixa')))
            
            observacao_edit = st.text_area("Observações", value=dados_cte.get('observacao', ''))
...
def main():
    ...
    with st.sidebar:
        ...
        # NOVO SISTEMA DE DOWNLOADS - COM TRATAMENTO DE ERRO
        if st.button("📊 Gerar Relatórios"):
            if not df_test.empty:
                with st.expander("📥 Opções de Download", expanded=True):
                    ...
                    with col_btn1:
                        ...
                        try:
                            ...
                            st.info("💡 Tente novamente ou use o download HTML")
                    
                    with col_btn2:
                        # Download HTML (para PDF) - COM TRATAMENTO DE ERRO
                        try:
                    ...
                    st.info("💡 **Conversão HTML → PDF:** Abra o arquivo HTML no navegador e use Ctrl+P → 'Salvar como PDF'")
